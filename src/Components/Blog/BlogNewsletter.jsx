...
export default function BlogNewsletter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://formsubmit.co/ajax/connect@adsserv.in', {
        ...
        body: JSON.stringify({
          _subject: `New Newsletter Subscriber (Blog): ${email.trim()}`,
          "Subscriber Email": email.trim(),
          "Subscription Type": "ADSSERV Blog Newsletter",
          "_template": "table",
          "_captcha": "false"
        })
      });
      ...
      console.error('Blog newsletter submission error:', err);
  ...
  return (
    <section ref={sectionRef} className="w-full bg-white py-24 md:py-32 font-sans flex flex-col items-center justify-center text-center px-4 overflow-hidden relative border-t border-slate-100">
      {/* Subtle bg accent */}
      ...
      <div className="max-w-[1200px] w-full mx-auto flex flex-col items-center relative z-10">
        ...
        <div ref={headlineRef} className="flex flex-col items-center w-full">
          ...
          <ScrollRevealHeading
            ...
            maxW="750px"
            className="mb-4"
            style={{ fontSize: 'clamp(2.2rem, 4.5vw, 4rem)', textAlign: 'center', lineHeight: 1.15 }}
            words={[
              { text: "Get" },
              { text: "Practical" },
        ...
        <div ref={formRef} className="flex flex-col items-center w-full">
          <p className="text-[15px] max-w-lg mb-8 leading-relaxed font-medium" style={{ color: 'rgba(30,47,87,0.65)' }}>
          ...
          {/* Subscribe form - exact HomePage Email format */}
          <form onSubmit={handleSubmit} className="w-full max-w-[600px] flex items-stretch shadow-xl overflow-hidden" style={{ borderRadius: 16, border: '1px solid rgba(30,47,87,0.12)' }}>
            <div className="flex items-center gap-3 flex-1 px-6 bg-white">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1e2f57" strokeWidth="2" strokeOpacity="0.4">
              ...
              <input
                ...
                required
                disabled={loading}
                className="flex-1 bg-transparent outline-none font-medium text-[14px] py-5 text-[var(--brand-navy)] placeholder:text-slate-400 disabled:opacity-50"
              />
            </div>
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-3 font-bold uppercase tracking-wider text-[13px] px-8 py-5 transition-all duration-300 hover:opacity-90 flex-shrink-0 disabled:opacity-60 cursor-pointer"
              style={{ background: '#1e2f57', color: '#fff' }}
              onMouseEnter={e => { if (!loading) e.currentTarget.style.background = 'var(--brand-orange, #e08326)'; }}
          ...
          <p className="mt-4 text-[12px] tracking-wide" style={{ color: 'rgba(30,47,87,0.5)' }}>
